...
if (!layout) {
    // Nothing in sessionStorage — send the user back to step 1.
...
function buildClueInputs(row, col) {
    ...
    acrossInput.setAttribute('aria-label', `Across clue at row ${row}, col ${col}`);
...
function buildPuzzle(layout) {
    ...
    for (let r = 0; r < rows; r++) {
        for (let c = 0; c < cols; c++) {
            ...
            const acrossRaw = document.getElementById(`across_${r}_${c}`)?.value ?? '0';
